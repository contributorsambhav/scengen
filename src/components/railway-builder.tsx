...
import { Calculator, Download, FileDown, GitBranch, Globe, Hand, Loader2, MapPin, Move, Play, Plus, Pointer, Route, Save, Settings, Train, Trash2, ZoomIn, ZoomOut } from 'lucide-react';
...
export default function RailwayBuilder({ onComputeComplete }: RailwayBuilderProps) {
  ...
  if (stations.length === 0 || sections.length === 0 || trains.length === 0) {
    alert('Please add at least one station, section, and train before computing paths');
    return;
  }
  ...
  try {
      ...
      allowed_sections: trains.reduce(
        (acc, t) => ({
          ...
          [t.id]: exportSections.reduce((sacc, s) => {
            const originalSection = sections.find((orig) => 
              (orig.from === s.from && orig.to === s.to) || 
              (orig.bidirectional && orig.from === s.to && orig.to === s.from)
            );
    ...
    alert(`Error computing path: ${error instanceof Error ? error.message : 'Unknown error'}`);
  } finally {
  ...
  const handleMouseMove = useCallback(
    ...
    [isDragging, draggedStation, isPanning, handlePanMove, screenToCanvas, connectingSection]
  );
  ...
  }, [isDragging, isPanning, connectingSection, handleMouseMove, handleMouseUp]);
  useEffect(() => {
      ...
      setTrains(
        trains.map((t) =>
          t.id === selectedItem.id
            ? {
                ...t,
                [property]: property.includes('Station') ? parseInt(value) : parseFloat(value) || 0
              }
            : t
        )
      );
  ...
  return (
    <div className="w-full h-screen flex bg-neutral-900 text-white">
      <div className="w-70 bg-neutral-800 overflow-y-auto hide-scrollbar">
        <h1 className={`text-xl px-4 border-b border-neutral-700 py-3 font-bold mb-6 sticky bg-neutral-800 top-0  ${instrumentSerif.className}`}>Scenegen Railway Builder</h1>
        <div className="p-4 space-y-6 pt-0">
          <div className="mb-6">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <button onClick={() => setSelectedTool('section')} className={`p-3 flex flex-col items-center gap-1 transition-all ${selectedTool === 'section' ? 'bg-white text-black' : 'bg-neutral-700 hover:bg-neutral-600'}`}>
                <GitBranch className="size-3.5" />
                <span className="text-xs">Section</span>
              </button>
              <button onClick={addTrain} className="p-3 bg-neutral-700 hover:bg-neutral-600 flex flex-col items-center gap-1 transition-all">
...
<div className="mb-6">
  <Button 
    onClick={computePath} 
    disabled={isComputing || stations.length === 0 || sections.length === 0 || trains.length === 0}
    className="w-full h-10 bg-green-600 hover:bg-green-700 disabled:bg-neutral-700 disabled:cursor-not-allowed flex items-center justify-center rounded-none gap-2 transition-all font-medium"
  >
    {isComputing ? (
      <>
      ...
      <>
        ...
        <span>Compute Path</span>
          ...
          <div className="mb-6">
            <h3 className="text-sm uppercase text-neutral-400 mb-2">View Controls</h3>
            <div className="space-y-2">
              <div className="text-xs text-neutral-300 mb-2 text-center">Zoom: {(zoom * 100).toFixed(0)}%</div>
              <div className="grid grid-cols-3 gap-2">
                <Button onClick={zoomIn} variant="outline" className="p-2 text-black rounded-none flex items-center justify-center gap-1 transition-all" title="Zoom In">
                  <ZoomIn className="size-4" />
                </Button>
                <Button onClick={zoomOut} variant="outline" className="p-2 text-black rounded-none flex items-center justify-center gap-1 transition-all" title="Zoom Out">
                  <ZoomOut className="size-4" />
                </Button>
                <Button onClick={resetView} variant="outline" className="p-2 text-black rounded-none flex items-center justify-center gap-1 transition-all text-xs" title="Reset View">
                  1:1
                </Button>
              </div>
              <Button onClick={() => setPanMode(!panMode)} className={`w-full p-2 rounded-none flex h-8 font-light text-xs items-center justify-center gap-2 transition-all ${panMode ? 'bg-orange-600 hover:bg-orange-700' : 'bg-neutral-700 hover:bg-neutral-600'}`}>
                <Move size={16} />
                <span className="text-sm">{panMode ? 'Pan Mode ON' : 'Pan Mode OFF'}</span>
              </Button>
              <div className="text-xs text-neutral-400 text-center">Zoom: {(zoom * 100).toFixed(0)}%</div>
            </div>
          </div>
          {}
          <div className="mb-6">
          ...
          {selectedItem && (
            <div className="mb-6 p-3 bg-neutral-900">
              <div className="flex justify-between items-center mb-3">
                <h3 className="text-xs uppercase text-neutral-300">{selectedItem.type} Properties</h3>
                <button onClick={deleteSelected} className="p-1 bg-red-600 hover:bg-red-700">
              ...
              {selectedItem.type === 'station' && (
                <div className="space-y-2">
                  <div>
                    ...
                    <Input type="text" value={stations.find((s) => s.id === selectedItem.id)?.name || ''} onChange={(e) => updateSelectedItem('name', e.target.value)} className="w-full p-2 bg-neutral-800 focus-visible:ring-0 border-0 rounded-none text-sm" placeholder="Station Name" />
                  </div>
                </div>
              )}
              {selectedItem.type === 'section' && (
                <div className="space-y-3">
                  ...
                  <div>
                    <Label className="text-xs text-neutral-400 font-normal">Distance (km)</Label>
                    <Input type="number" value={sections.find((s) => s.id === selectedItem.id)?.distance || 0} onChange={(e) => updateSelectedItem('distance', e.target.value)} className="w-full p-2 focus-visible:ring-0 bg-neutral-800 rounded-none border-0 text-sm" step="0.1" min="0.1" />
                  </div>
                  <div>
                    <Label className="text-xs text-neutral-400 font-normal">Max Speed (km/h)</Label>
                    <Input type="number" value={sections.find((s) => s.id === selectedItem.id)?.maxSpeed || 0} onChange={(e) => updateSelectedItem('maxSpeed', e.target.value)} className="w-full p-2  focus-visible:ring-0 bg-neutral-800 rounded-none border-0 text-sm" step="10" min="10" />
                  </div>
                  <div className="flex items-center space-x-2">
                    <Input type="checkbox" checked={sections.find((s) => s.id === selectedItem.id)?.bidirectional || false} onChange={(e) => updateSelectedItem('bidirectional', e.target.checked)} className="rounded size-2.5" id="bidirectional" />
                    <Label htmlFor="bidirectional" className="text-xs text-neutral-400">
                      Bidirectional
                    </Label>
                  </div>
                </div>
              ...
              {selectedItem.type === 'train' && (
                <div className="space-y-2">
                  <div>
                    <Label className="text-xs text-neutral-400">Max Speed (km/h)</Label>
                    <Input type="number" value={trains.find((t) => t.id === selectedItem.id)?.maxSpeed || 0} onChange={(e) => updateSelectedItem('maxSpeed', e.target.value)} className="w-full p-2 focus-visible:ring-0 bg-neutral-800 rounded-none border-0 text-sm" step="10" />
                  </div>
                  <div>
                    <Label className="text-xs text-neutral-400">Start Station</Label>
                    <select value={trains.find((t) => t.id === selectedItem.id)?.startStation || ''} onChange={(e) => updateSelectedItem('startStation', e.target.value)} className="w-full p-2 bg-neutral-800 rounded text-sm">
                      {stations.map((s) => (
                  ...
                  <div>
                    <Label className="text-xs text-neutral-400">End Station</Label>
                    <select value={trains.find((t) => t.id === selectedItem.id)?.endStation || ''} onChange={(e) => updateSelectedItem('endStation', e.target.value)} className="w-full p-2 bg-neutral-800 rounded text-sm">
                      {stations.map((s) => (
                        <option key={s.id} value={s.id}>
                          {s.name}
                        </option>
          ...
          {sections.length > 0 && (
            <div className="mb-6">
              <h3 className="text-sm uppercase text-neutral-400 mb-2">Sections</h3>
              <div className="space-y-1 max-h-32 overflow-y-auto hide-scrollbar">
                {sections.map((section) => {
                  const info = getSectionInfo(section);
                  return (
                    <div key={section.id} onClick={() => setSelectedItem({ type: 'section', id: section.id })} className={`p-2 cursor-pointer  text-xs transition-all ${selectedItem?.type === 'section' && selectedItem.id === section.id ? 'bg-neutral-900 text-white' : 'bg-neutral-700 hover:bg-neutral-600'}`}>
          ...
          {}
          <div className="mb-6">
            <h3 className="text-sm uppercase text-neutral-400 mb-2">Network Overview</h3>
            <div className="space-y-1 text-sm">
              ...
              <div className="flex justify-between">
                <span className="text-neutral-400 text-xs">Sections (Export):</span>
          ...
          {}
          <div className="mb-6">
            <h3 className="text-sm uppercase text-neutral-400 mb-2">Global Settings</h3>
            <div className="space-y-2">
              <div>
                <Label className="text-xs text-neutral-400">Headway (min)</Label>
                <Input type="number" value={headway} onChange={(e) => setHeadway(parseFloat(e.target.value) || 0)} className="w-full p-2 shadow-none focus-visible:ring-0 bg-neutral-700 rounded-none border-0 text-sm" step="0.5" />
              </div>
              <div>
                <Label className="text-xs text-neutral-400">BIG_M</Label>
                <Input type="number" value={bigM} onChange={(e) => setBigM(parseInt(e.target.value) || 10000)} className="w-full p-2 shadow-none focus-visible:ring-0 bg-neutral-700 rounded-none border-0 text-sm" step="1000" />
              </div>
            </div>
          </div>
          {}
          {trains.length > 0 && (
            <div className="mb-6">
              <h3 className="text-sm uppercase text-neutral-400 mb-2">Trains</h3>
              <div className="space-y-2">
                {trains.map((train) => (
                  <div key={train.id} onClick={() => setSelectedItem({ type: 'train', id: train.id })} className={`p-2 cursor-pointer transition-all ${selectedItem?.type === 'train' && selectedItem.id === train.id ? 'bg-neutral-900 text-white' : 'bg-neutral-700 hover:bg-neutral-600'}`}>
                    <div className="flex justify-between items-center">
                      <span className="font-medium text-sm">Train {train.id}</span>
                      <span className="text-xs text-neutral-300">{train.maxSpeed} km/h</span>
                    </div>
                    ...
                    {selectedItem?.type === 'train' && selectedItem.id === train.id && sections.length > 0 && (
                      <div className="mt-2 pt-2 border-t border-neutral-600">
                        <div className="text-xs text-neutral-400 mb-1">Allowed Sections:</div>
          ...
          <Button onClick={exportData} disabled={stations.length === 0 || sections.length === 0} className="w-full h-8 border border-neutral-700/80 font-normal text-xs  disabled:bg-neutral-700 disabled:cursor-not-allowed flex items-center justify-center rounded-none gap-2 transition-all">
      ...
      <div className="flex-1 relative bg-neutral-950 overflow-hidden">
        ...
        <div
          ...
          style={{
            cursor: selectedTool === 'station' ? 'crosshair' : panMode ? 'move' : isDragging ? 'grabbing' : 'default'
          }}
        >
          {}
          <div
          ...
          >
            ...
            >
              ...
              {}
              {connectingSection && <line x1={getRawStationPos(connectingSection.from).x + 5000} y1={getRawStationPos(connectingSection.from).y + 5000} x2={mousePos.x + 5000} y2={mousePos.y + 5000} stroke="#06b6d4" strokeWidth="2" strokeDasharray="5,5" opacity="0.5" className="pointer-events-none" />}
            </svg>
            {}
            {stations.map((station) => (
              <div
                ...
                style={{
                  left: station.x,
                  top: station.y,
                  cursor: selectedTool === 'section' ? 'pointer' : selectedTool === 'select' ? (draggedStation === station.id ? 'grabbing' : 'grab') : 'default'
                }}
              >
                <div className={`relative ${connectingSection?.from === station.id ? 'animate-pulse' : ''}`}>
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center transition-all ${selectedItem?.type === 'station' && selectedItem.id === station.id ? 'bg-white text-black shadow-lg shadow-cyan-600/50' : connectingSection?.from === station.id ? 'bg-yellow-600' : 'bg-neutral-700 hover:bg-neutral-600'}`}>
                    <MapPin size={20} />
                  </div>
                  <div className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 text-xs whitespace-nowrap bg-neutral-800 px-2 py-1 rounded">{station.name}</div>
                </div>
              </div>
            ))}
          </div>
