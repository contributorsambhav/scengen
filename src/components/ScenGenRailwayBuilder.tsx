...
;
  ...
  const exportData = () => {
    ...
    const data: NetworkData = {
      ...
      allowed_sections: trains.reduce((acc, t) => ({ 
        ...acc, 
  ...
  const updateSelectedItem = (property: string, value: any) => {
    ...
    if (selectedItem.type === 'station') {
      ...
      setTrains(trains.map(t => 
        t.id === selectedItem.id ? { ...t, [property]: property.includes('Station') ? parseInt(value) : parseFloat(value) || 0 } : t
      ));
    }
  };
      ...
      try {
        ...
        Object.entries(data.sec_from).forEach(([sectionId, from]) => {
          ...
          if (!processedPairs.has(pairKey)) {
            ...
            const reverseSection = Object.entries(data.sec_from).find(([revId, revFrom]) => 
              revId !== sectionId && revFrom === to && data.sec_to[revId] === from
            );
        ...
        const importedTrains: TrainConfig[] = Object.entries(data.start_station).map(([trainId, startStation]) => ({
          ...
          allowedSections: importedSections.reduce((acc, s) => {
            ...
            const isAllowed = Object.entries(data.sec_from).some(([expSecId, expFrom]) => {
              ...
              const sectionMatches = (s.from === expFrom && s.to === expTo) ||
    ...
    <div className="w-full h-screen flex bg-gray-900 text-white">
      ...
      <div className="w-80 bg-gray-800 p-4 overflow-y-auto">
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              onClick={() => { setSelectedTool('select'); setConnectingSection(null); }}
              className={`p-3 rounded flex flex-col items-center gap-1 transition-all ${
                selectedTool === 'select' ? 'bg-cyan-600' : 'bg-gray-700 hover:bg-gray-600'
              }`}
            >
              <Settings size={20} />
            ...
            <button
              onClick={() => { setSelectedTool('station'); setConnectingSection(null); }}
              className={`p-3 rounded flex flex-col items-center gap-1 transition-all ${
                selectedTool === 'station' ? 'bg-cyan-600' : 'bg-gray-700 hover:bg-gray-600'
              }`}
            >
              <MapPin size={20} />
            ...
            <button
              ...
              className={`p-3 rounded flex flex-col items-center gap-1 transition-all ${
                selectedTool === 'section' ? 'bg-cyan-600' : 'bg-gray-700 hover:bg-gray-600'
              }`}
            >
              <GitBranch size={20} />
            ...
            <button
              onClick={addTrain}
              className="p-3 rounded bg-gray-700 hover:bg-gray-600 flex flex-col items-center gap-1 transition-all"
            >
              <Train size={20} />
            ...
            {selectedItem.type === 'station' && (
              <div className="space-y-2">
                <div>
                  ...
                  <input
                    type="text"
                    value={stations.find(s => s.id === selectedItem.id)?.name || ''}
            ...
            {selectedItem.type === 'section' && (
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={sections.find(s => s.id === selectedItem.id)?.distance || 0}
                    onChange={(e) => updateSelectedItem('distance', e.target.value)}
                    className="w-full p-2 bg-gray-800 rounded text-sm"
                    step="0.1"
                    min="0.1"
                  />
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={sections.find(s => s.id === selectedItem.id)?.maxSpeed || 0}
                    onChange={(e) => updateSelectedItem('maxSpeed', e.target.value)}
                    className="w-full p-2 bg-gray-800 rounded text-sm"
                    step="10"
                    min="10"
                  />
                ...
                <div className="flex items-center space-x-2">
                  <input
                    type="checkbox"
                    checked={sections.find(s => s.id === selectedItem.id)?.bidirectional || false}
                    onChange={(e) => updateSelectedItem('bidirectional', e.target.checked)}
                    className="rounded"
                    id="bidirectional"
                  />
            ...
            {selectedItem.type === 'train' && (
              <div className="space-y-2">
                <div>
                  ...
                  <input
                    type="number"
                    value={trains.find(t => t.id === selectedItem.id)?.maxSpeed || 0}
                    onChange={(e) => updateSelectedItem('maxSpeed', e.target.value)}
                    className="w-full p-2 bg-gray-800 rounded text-sm"
                    step="10"
                  />
                ...
                <div>
                  ...
                  <select
                    value={trains.find(t => t.id === selectedItem.id)?.startStation || ''}
                    onChange={(e) => updateSelectedItem('startStation', e.target.value)}
                    className="w-full p-2 bg-gray-800 rounded text-sm"
                  >
                    {stations.map(s => (
                ...
                <div>
                  ...
                  <select
                    value={trains.find(t => t.id === selectedItem.id)?.endStation || ''}
                    onChange={(e) => updateSelectedItem('endStation', e.target.value)}
                    className="w-full p-2 bg-gray-800 rounded text-sm"
                  >
                    {stations.map(s => (
                      <option key={s.id} value={s.id}>{s.name}</option>
                ...
                return (
                  <div
                    ...
                    className={`p-2 rounded cursor-pointer text-xs transition-all ${
                      selectedItem?.type === 'section' && selectedItem.id === section.id
                        ? 'bg-cyan-600'
          ...
          <div className="mb-6">
            ...
            <div className="space-y-2">
              {trains.map(train => (
                <div
                  key={train.id}
                  onClick={() => setSelectedItem({ type: 'train', id: train.id })}
                  className={`p-2 rounded cursor-pointer transition-all ${
                    selectedItem?.type === 'train' && selectedItem.id === train.id
                      ? 'bg-cyan-600'
                      : 'bg-gray-700 hover:bg-gray-600'
                  }`}
                >
                  ...
                  <div className="text-xs text-gray-300 mt-1">
                    {stations.find(s => s.id === train.startStation)?.name || 'Unknown'} → 
                    {stations.find(s => s.id === train.endStation)?.name || 'Unknown'}
                  </div>
        ...
        <button
          ...
          className="w-full p-3 bg-green-600 hover:bg-green-700 disabled:bg-gray-700 disabled:cursor-not-allowed rounded flex items-center justify-center gap-2 transition-all"
        >
          <FileDown size={20} />
          Export JSON
        </button>
      ...
      <div className="flex-1 relative bg-gray-950">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            {selectedTool === 'section' && (
              <>
                ...
                <span>
                  {connectingSection ? 'Click destination station' : 'Click source station'}
                </span>
              </>
            )}
            {selectedTool === 'select' && (
        ...
        <div
          ...
          style={{ cursor: selectedTool === 'station' ? 'crosshair' : 'default' }}
        >
          {/* Render Sections */}
          <svg className="absolute inset-0 w-full h-full pointer-events-none">
            ...
            {connectingSection && canvasRef.current && (
              <line
                x1={getStationPos(connectingSection.from).x}
                y1={getStationPos(connectingSection.from).y}
                x2={0}
                y2={0}
                stroke="#06b6d4"
                strokeWidth="2"
          ...
          {stations.map(station => (
            <div
              ...
              className={`absolute transform -translate-x-1/2 -translate-y-1/2 transition-all ${
                selectedItem?.type === 'station' && selectedItem.id === station.id
                  ? 'scale-110'
                  : ''
              }`}
              style={{
            ...
            >
              <div className={`relative ${
                connectingSection?.from === station.id ? 'animate-pulse' : ''
              }`}>
                ...
                <div className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 text-xs whitespace-nowrap bg-gray-800 px-2 py-1 rounded">
                  {station.name}
                </div>
              </div>
            </div>
